...
| MCP | Stdio server, optional read-only policy, protocol/subprocess tests, validated macOS MCPB and candidate registry metadata | Actual client compatibility matrix, permissions under client launch, complete calendar subprocess coverage |
...
| Installation | Source-build script, ad-hoc helper signing, LaunchAgent template, versioned macOS tarball and MCPB build paths, atomic executable switching with rollback | Stable signed/notarized application, permission continuity, uninstall workflow, architecture matrix and installed lifecycle evidence |
