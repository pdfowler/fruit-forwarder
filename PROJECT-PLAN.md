...
The `check`, `build:macos`, `package:ha`, `package:macos`, `package:mcp`, `install:local`, `release:check`, `release:prepare` and guarded `publish:*` interfaces are implemented in `Taskfile.yml`; the package targets now build a versioned macOS tarball, a validated MCPB and a deterministic HACS export, while signing, publication and live installation remain explicit maintainer actions. The publication wrappers perform clean exact-tag and attribution preflight, then dispatch the protected GitHub workflows; they do not contain credentials or publish implicitly. `task build:macos` is the shared staged Go/native build path used by macOS and MCP packaging; both native executables are signed with `FRUIT_FORWARDER_SIGN_IDENTITY` (ad-hoc by default, Developer ID required for a publishable release), and the selected identity is recorded in the release manifest. `task install:local PACKAGE=...` verifies the archive and checksum, rejects unsafe tar members, extracts only to a temporary directory, and invokes the packaged transactional installer; optional migration and install-only flags are explicit Task variables. `task check` also runs `scripts/check-git-attribution.sh`, which verifies the requested author and committer identity across all refs before a candidate can pass. On macOS, `task release:check` now builds and verifies every local artifact before `task release:prepare` writes the manifest; on other hosts it reports the host-specific omission explicitly. Run independent tests/builds in parallel; express real dependencies in order. A shared protocol or fixture change must invalidate checks for Go, HA and MCP consumers. A shared native change must rebuild both Mac and MCP distributions. Changes limited to HA should not require an unrelated native rebuild unless compatibility or release policy requires it.
...
6. Record the upstream source commit and dirty-tree state in the release manifest. Make generation reproducible; the HACS export also records a deterministic tree digest and the publisher rejects a destination whose generated contents were edited or replaced unexpectedly. Route fixes back through the canonical repository.
...
3. The new per-user LaunchAgent invokes the bridge through `launchctl asuser <uid>` in its `gui/<uid>` domain. Live Dean testing showed the direct arrangement can time out in EventKit while the same binary succeeds interactively and through `asuser`; retain this wrapper until fresh-permission and lifecycle testing proves it unnecessary. Keep legacy migration separate.
