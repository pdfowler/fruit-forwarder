...
| HA integration | Setup, todo/calendar entities, persistent queue, scope filtering, save rollback | HTTP webhook tests, duplicate-entry handling, robust reconfiguration, stale/failed operation UX |
| MCP | Stdio server, optional read-only policy, protocol and subprocess tests | Actual client compatibility matrix, permissions under client launch, complete calendar subprocess coverage |
...
| Installation | Source-build script, ad-hoc helper signing, LaunchAgent template | Stable packaged application, atomic upgrades, permission continuity, uninstall/rollback, architecture matrix |
...
| Sharing | README, MIT license, security/contributor docs and migration notes | Final project identity, complete privacy/license review, remote repository, release assets and real security-reporting channel |
...
**MCP distribution:** build the MCP server from the main repository and supply a separate listing, configuration guide and supported installation artifact. The official MCP Registry hosts metadata, not the executable. Select and validate an appropriate supported package route for the native macOS software; do not assume a GitHub binary release alone constitutes a registry-installable package. A candidate registry identifier is `io.github.pdfowler/fruit-forwarder`, conditional on publishing through that verified GitHub namespace. Registry naming, package naming and source repository naming are distinct; none requires duplicating the project into a third repository.
...
The initial `check`, `package:ha`, `package:macos`, `package:mcp` and `release:check` interfaces are implemented in `Taskfile.yml`; `package:mcp` remains an explicit release note because the final MCPB route is still a maintainer decision. Run independent tests/builds in parallel; express real dependencies in order. A shared protocol or fixture change must invalidate checks for Go, HA and MCP consumers. A shared native change must rebuild both Mac and MCP distributions. Changes limited to HA should not require an unrelated native rebuild unless compatibility or release policy requires it.
...
#### Release and HACS export pipeline
...
4. Export only allowlisted HA files into a temporary distribution tree with root-level `custom_components/<domain>/`, `hacs.json`, required metadata/brand assets, license notices and HA README. Validate the exported tree itself with HACS/Hassfest and check installation from it.
5. Record the upstream source commit in the export. Make generation reproducible; detect unexpected distribution-repository edits rather than overwriting them silently. Route fixes back through the canonical repository.
6. Publish through a narrowly scoped automation identity only after the release gates pass and publication is authorized. Prefer ordinary auditable commits/releases in the distribution repo over force-pushing its history. Preserve the requested `pfowler@icloud.com` commit attribution in repository export configuration, and distinguish commit authorship from release automation provenance.
7. Publish Mac artifacts, the chosen MCP package/registry metadata, and the HA distribution/release with compatibility links. Cross-service publication is not atomic: record per-target status and support retry without replacing immutable released versions.
8. If a target fails, hold or mark the release partial rather than advertising full availability. Maintain a manifest of the last known compatible set and documented rollback behavior.
...
- Make discovery honor supported config/helper-path options; currently discovery uses the default helper path independently of normal config loading.
