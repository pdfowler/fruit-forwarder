...
| Protocol | Version 1 with additive capabilities | Mac bridge and HA integration must use matching protocol version; calendar clients fail closed when `calendars` is not advertised, while legacy reminder-only responses remain compatible |
