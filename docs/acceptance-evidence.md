...
| R7 | Approachable installation/upgrades | Partial | Versioned archive, installer, rollback scripts, setup documentation, and tested `status`/`doctor` diagnostics | Clean-user install, upgrade and rollback from the packaged artifact |
...
| R9 | Shareable open-source distribution | Partial | MIT license, CI, contributor/security docs, deterministic HACS export, guarded HACS publication workflow, macOS/MCP candidates | Authorized public repositories, release assets, support channel and ecosystem publication |
